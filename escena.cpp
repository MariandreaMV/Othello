    ...
    QGraphicsScene(parent)
{
    ...
    QPixmap *p = new QPixmap("./Imagen/hojas.jpg");
    this->setBackgroundBrush(p->scaled(64*11+25,64*8+45, Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
    ...
    aux.setPixelSize(40);
    aux.setBold(true);
    aux.setFamily("Fixedsys");

    fuente2.setPixelSize(25);
    fuente2.setBold(true);
    fuente2.setFamily("Fixedsys");

    puntos_texto2 = new QGraphicsTextItem;
    puntos_texto2->setFont(aux);
    puntos_texto2->setPos(518,18);
    puntos_texto2->setPlainText("Puntos: ");
    puntos_texto2->setDefaultTextColor(Qt::black);

    puntos_texto = new QGraphicsTextItem;
    puntos_texto->setFont(fuente);
    puntos_texto->setPos(520,20);
    puntos_texto->setPlainText("Puntos: ");
    puntos_texto->setDefaultTextColor(QColor(35,135,35));
    ...
    puntos_jugador2->setGraphicsEffect(effect);
    if (opc == 1)
    ...
    turno_jugador->setGraphicsEffect(effect);
                    ...
                    actualizar();
                     if (juego->getMovimientosPosibles() == 0) {
                         ...
                         if (juego->getMovimientosPosibles() == 0) {
                             ...
                             actualizar();
                             if (juego->getJugadorActual() == 1)
                                 jugadas_mostrar->setPlainText("J1\npasa turno");
                             else
                                 jugadas_mostrar->setPlainText("J2\npasa turno");
                         }
                     }
           //--------------FIN HUMANO VS HUMANO-------------------------//
                        ...
                        if (juego->getMovimientosPosibles() != 0) {
                            ...
                            if (juego->getMovimientosPosibles() == 0) {
                                ...
                                if (juego->getJugadorActual() == 1)
                                    ...
                                    jugadas_mostrar->setPlainText("PC\nsede turno");
                            }
                            actualizar();
                        }
                    }
                    if (juego->getJugadorActual() == 2){//juega maquina
                        if (juego->getMovimientosPosibles() != 0) {
                            ...
                            Casilla* ideal;
                            if (dificultad == 1)
                                ideal = minMax->jugadaRandom();//juega la inteligencia random
                            if (dificultad == 2)
                                ideal = minMax->mejorJugada();//juega la inteligencia dificil
                            if (ideal != NULL){
                                x = ideal->getColu();
                                y = ideal->getFila();
                                juego->turno(y,x);
                            ...
                            jugadas_mostrar->setPlainText("PC cede\nturno");
                            juego->cambiarTurnos();
                        }
                        if (juego->getMovimientosPosibles() == 0) {
                            juego->cambiarTurnos();
...
void Escena::actualizar()
{
    ...
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
                this->addRect(x,y,64,64,QPen(Qt::gray),QBrush(QColor(35,135,35)));
